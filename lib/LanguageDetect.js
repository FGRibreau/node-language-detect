...
var db_lang = require('../data/lang.json');
...
LanguageDetect.prototype = {
  ...
  _distance:function (arr1, arr2) {
    ...
    for (var i = 0, iM = keys.length; i < iM; i++) {
      ...
      if (arr1[keys[i]]) {
        distance = Math.abs(value - arr1[keys[i]]);
      } else {
   ...
   * @param   _base_count   the number of trigrams being considered
   ...
   * @param   _limit  if specified, return an array of the most likely
